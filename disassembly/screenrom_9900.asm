...
.old_rdcvec
    ...
    equb &0d, &ff, &ff, &ff, &ff, &ff, &ff, &ff, &ff, &ff, &ff
    ...
    equb &ff
.pydis_end
    assert <(basic_next_statement) == &5b
    assert <(ca8c0) == &c0
    assert <(ca8e2) == &e2
    assert <(cab7e) == &7e
    assert <(cab8d) == &8d
    assert <font_ram == &08
    assert <rdch == &95
    assert <rdch_with_delay == &2a
    assert <workspace_ram == &00
    ...
    assert <wrch_vdu28d == &9f
    assert <wrch_vdu31a == &3b
    assert <wrch_vdu31b == &5f
    assert >(basic_next_statement) == &c5
    assert >(ca8c0) == &a8
    assert >(ca8e2) == &a8
    assert >(cab7e) == &ab
    assert >(cab8d) == &ab
    ...
    assert >font_ram == &99
    assert >rdch == &a3
    assert >rdch_with_delay == &98
    assert >workspace_ram == &99
    ...
    assert >wrch_vdu28d == &a4
    assert >wrch_vdu31a == &a5
    assert >wrch_vdu31b == &a5
    assert bget_fast == &ab72
    assert bget_slow == &ab30
    assert bput_fast == &ab78
    assert bput_slow == &aaef
    assert kern_BEL == &fd1a
    assert kern_load == &f96e
    assert kern_nvrdch == &fe94
    assert kern_nvwrch == &fe52
    assert kern_save == &fae5
    assert kern_send_A_to_printer == &fefb
    assert load_fast == &accb
    assert save_fast == &acd1
    assert vdu01_entry == &a519
    assert vdu04_entry == &a5d3
    assert vdu05_entry == &a5dc
    assert vdu06_entry == &a7f9
    assert vdu08_entry == &a67c
    assert vdu09_entry == &a63c
    assert vdu10_entry == &ac20
    assert vdu11_entry == &a66c
    assert vdu12_entry == &a619
    assert vdu13_entry == &a86c
    assert vdu14_entry == &a5e1
    assert vdu15_entry == &a5e5
    assert vdu17_entry == &a5ea
    assert vdu18_entry == &a5f0
    assert vdu19_entry == &a5f6
    assert vdu20_entry == &a7dc
    assert vdu21_entry == &a5ff
    assert vdu22_entry == &a576
    assert vdu23_entry == &a4cb
    assert vdu26_entry == &a604
    assert vdu28_entry == &a417
    assert vdu30_entry == &a65b
    assert vdu31_entry == &a530
    assert vdu_code_lookup - vdu_code_table == &4e
